...
### 💾 数据备份
- 数据库备份
- 媒体文件备份（备件图片、办公机配件图片等）
- 备份文件管理
- 数据恢复（支持上传SQL文件恢复，自动处理外键约束）
- 媒体文件恢复（支持上传.tar.gz文件恢复）
- 自动清理过期备份（可配置保留数量）
...
媒体文件备份包含备件图片、办公机配件图片等上传的文件。

**备份**：
- 自动：数据库备份时会自动备份媒体文件（可配置开关）
- 备份文件格式：`.tar.gz`

**恢复**：
1. 进入 **数据备份** → **数据恢复** 页面
2. 在媒体文件恢复区域上传 `.tar.gz` 备份文件
3. 点击 **恢复媒体文件**
4. 系统会自动备份现有文件到 `media/backup_时间戳/` 目录
